...
| 1.8 | Shape-keyed store cells (L2 slice b): remove the >4096-object store cliff with a (map id, name) write cell | **landed 2026-09-04** (this tree) | The (id, name) `MEMBER_WRITE_CELLS` table thrashes once a store loop's object working set exceeds 4096, even when every object shares one shape — measured cliff (200k-call rows, fresh release build of `ebe30cc`): 8192-object stores ~181ns/store interp (~162 jit) and 16384 ~182ns vs ~56ns/~40ns at 64-1024 objects. Fix: a second direct-mapped table `member_write_map_cells` keyed by (map id, name) — a map id pins the descriptor layout for every instance of the shape, so a hit needs no per-object identity or generation. Probed as the fallback when the (id, name) cell misses; recorded (in `warm_store_record`) ONLY for map-described inline keys (a vector-only property's slot is per-object — two objects can share a live map yet hold different vectors after it), so the pinned inline mirror is always real and the `write_data_property_slot` stored-key recheck backstops a stale slot. A map-keyed hit also re-keys the (id, name) cell so the same instance's next store keeps the cheaper identity probe, and fronts the read-side value cell under the L1c no-bump discipline. 8192 stores ~181 -> ~58ns/interp (~162 -> ~44 jit) and 16384 ~182 -> ~67ns — the rows now sit AT the 64/1024 warm level (~11-13ms interp / ~8-9ms jit), and the single-object row is unchanged (no warm regression). Gates: clippy, workspace tests (new `stores_over_many_same_shape_objects_stay_exact` — 9000 same-shape instances with distinct values, interleaved map transitions and deletes, all read back), three sweeps at baseline (perf.md record). Follow-on: vector-only keys on shared maps still take the (id, name) table (their slot is not map-pinned); a >4096-object loop writing a 5th+ field of a many-field shape still cliffs — the per-STEP store IC (1.2) is the target there, and the chain-member-read slice (a) stays behind L1c's shape end-state. |
...
4. 3.1 (L4 arena) is CLOSED by its counting probe (2026-09-04): the arena
   ...
   cost. 1.8 (the >4096-object store ceiling, L2 slice b) is LANDED
   (2026-09-04, this tree): a second (map id, name)-keyed write table
   (`member_write_map_cells`) serves every instance of a shape at the
   map-pinned slot after the identity table thrashes — the 8192/16384-
   object store rows drop ~181-182ns -> ~58-67ns/store interp (~162 ->
   ~44ns jit) and sit at the 64/1024 warm level, with the single-object
   row unchanged. Remaining candidates, in
