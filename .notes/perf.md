...
Open (measured, not started): the residual ~35-38ms jit char_ops floor is the registered-method CALL side (~110ns/call, P1 jit ~25ms) — read and call both now near their floors; `charCodeAt`'s native body and the method-call dispatch are the next terms. A String-exotic read of a DEEP chain prop (a boxed receiver reading an Object.prototype method) still pays the full path per call.
