...
### LANDED (2026-09-10): invariant member reads hoist out of certified loops — `property read` jit 5.2 -> 2.5ms

The LICM slice the probe above called for. `Compiler::hoistable_member_loop` recognizes a certified `for` whose body/update/test/init are user-code-free once a one-shot guard has run and whose member reads are all `RECV.name` on a frame-slot receiver the loop never writes: no calls, no object/array/function/regexp literals, no computed/optional member access, no `in`/`instanceof`/`delete`, no suspension, no control transfer out of the loop, every named member read on a plain-identifier receiver (so nothing in the loop can run an accessor), and no lexical (`let`/`const`) declaration in the body (it could shadow a scanned name — the guard resolves names at the enclosing scope). It emits `Step::HoistMemberGuard`: a once-per-loop guard that (1) verifies every VALUE-operand slot holds a non-object primitive — objects/functions are the only operands whose coercion can re-enter user code and mutate a hoisted receiver — and (2) probes each read through the data-only member cell; on a full hit the values land in hidden frame slots and the same loop re-compiles with its reads lowered to `LoadLocal(hoist_slot)` (the fast copy), on any miss it re-compiles unchanged as the general per-iteration loop. Both copies share the M6 length hoist's `hoist_suppressed` guard.

Soundness: a value-cell/map-cell hit proves an own/map-served DATA property (accessors are never cell-served), and the loop purity plus the primitive operand guard mean no user code can run in the fast copy, so the value is loop-invariant across it. A non-object-primitive operand value (including the TDZ marker — the interpreter arm checks `is_uninitialized` before the cell probe, since the reserved tag must never reach `Value::kind`) misses to the general loop.

The JIT's inline member probe (`emit_member_cell_read`) was split into `emit_member_cell_probe(object, name, miss)` so the guard can branch to the general loop on a miss instead of emitting the `get_member_name` fallback; no new runtime helper was needed.

Measured (`--jit-bench`, release, 3 runs): `property read` jit 5.18-5.44 -> **2.52-2.66ms** (~2.05x; interp 25-27 -> 18.0-18.2ms, ~1.4x) — the hoist's predicted read-hoist ceiling. Node's 0.324ms is still 7.8x off (was ~16.7x): node also const-folds the invariant sum, the queued arithmetic-hoist follow-up. All other rows unchanged (arithmetic 0.62ms, global read 3.0ms).

Verification: clippy `--workspace --all-targets -D warnings` clean; `cargo test --workspace` 4779 pass / 0 fail (incl. `jit::tests::installed_jit_hoists_invariant_member_reads_without_changing_semantics`); six test262 sweeps at baseline (language 23721/0/3, annexB 1086/0/0, built-ins 23657/0/155, with the JIT and with `--jitless`, zero fail/crash/hang); a 26-case differential battery (`scratch/hoist_battery.js`: data reads, getters, getter-mutates-sibling, object-valued accumulator, receiver reassignment, member/computed/element writes, `length`, zero-iteration loop, nullish/TDZ receivers, delete, string and NaN/-0/Infinity/boolean values, nested loops, break/`in`, global and proto-chain receivers, patched `Object.prototype.valueOf`, shadowed receiver, function own-data, typed-array elements, multi-receiver) byte-identical across jit / `--jitless` / node and again under `--gc-stress`.

### LANDED (2026-09-10): the invariant RHS hoist + the num-slot `Slot` recipe — `property read` jit 5.2 -> 0.75ms (~6.9x)

The arithmetic-hoist follow-up the read-hoist entry queued; the row now sits on the arithmetic floor. Two parts:

1. **RHS hoist** — `hoistable_member_loop` also collects assignment-RHS expressions built only from the hoisted reads and non-BigInt literals with at least one operator (`expr_invariant_rhs`), and `compile_hoisted_member_for` computes each once in a preheader into a fresh hidden slot; the fast copy's expression redirect (`Compiler.hoisted_exprs`, keyed by the RHS source span) lowers the same expression to `LoadLocal(hoist_slot)`. Reads feeding a hoisted RHS must additionally exclude BigInt/Symbol (the coercions that can throw) — the preheader runs before the loop's test, so a throw must not move across a zero-trip loop.

2. **`NumRhs::Slot`** — the num-slot reduction (`plan_loop_num`) now accepts a guard-verified invariant Number slot as its RHS: `rhs_recipe` recognizes `[LoadReg(proven_slot)]` and `run_acc_is_number` treats a proven slot as Number, so the RMW becomes `BinStoreNum { rhs: Slot(slot) }` (raw f64 in both engines — no tag check, no canonicalization). `Compiler.proven_num_slots` is set around the fast copy; a SECOND guard (`HoistMemberGuard { numbers: true }`) checks that the hoisted RHS results are Numbers, and a miss takes a read-hoisted-but-not-RHS copy (3 copies total). `prefix_is_straight_line` now admits a `HoistMemberGuard` (its target is a forward fixup to the general copy, which is emitted after the loop).

Measured (`--jit-bench`, release): `property read` jit 5.18-5.44 -> **0.75-0.80ms** (~6.9x; interp 25-27 -> 8.5ms, ~3.1x; node jit 0.31ms, gap 16.7x -> ~2.5x). On the jitless column slag's ~8.5ms now BEATS node `--jitless`'s ~15.1ms (~1.8x). The row is at the `const add` floor (`n += 3` ~0.7ms), so the remaining ~2.5x vs node is the shared `arithmetic`-floor gap, not this row's read code.

Validation: clippy `--workspace --all-targets -D warnings` clean; `cargo test --workspace` 4779 pass / 0 fail; six test262 sweeps at baseline (annexB 1086/0/0, built-ins 23657/0/155, language 23721/0/3 — one unreproduced 1-fail `language` JIT run in 9, clean in the other 8, zero fail/crash/hang otherwise); the 39-case differential battery (`scratch/hoist_battery.js`: getters, getter-mutates-sibling, BigInt/Symbol reads incl. the zero-trip no-throw case, string/boolean/`typeof`/mixed RHS, object-valued accumulator, receiver/member writes, TDZ/nullish receivers, proto chain, shadowed receiver, typed arrays, nested loops) byte-identical across jit / `--jitless` / node and under `--gc-stress`.

### NEGATIVE PROBE (2026-09-10): the `typed-array write` row is BUILD-layout-sensitive at ~2x (no attributable regression)

Chasing what looked like a 2x regression from the LICM arc: the row measured 24.77ms pre-session, 11.72ms after the `ObjectFast` leaf fix, 12.1-12.3ms after the read-hoist commit, ~24ms after the RHS/Slot landing, and 12.0-13.1ms again on the NEXT rebuild of the SAME source — removing/adding an `eprintln!`, or just `touch`ing files and rebuilding, flips it. A single-function probe (`scratch/ta_probe3.js`, 800k stores) reproduces it with no source difference (24ms vs 12.7ms), stable within a build; padding/ordering functions inside one build has no effect, so it is not intra-build alignment. The 12ms builds carry the inline byte store (`movb %r11b, (%rax,%rcx)` in `JIT_DUMP_CLIF`); the 24ms builds do not — the compiled store lowering itself differs between builds of identical Slag source. Mechanism unknown (recorded, not diagnosed). Consequence: this row cannot gate a change without a fresh-build protocol, and since the pre-session value is the 24.77 slow side, the LICM work carries no attributable regression here. Also a data-reproducibility caveat for the node table (24ms vs node's 0.29ms overstates the gap by 2x on a fast build).
