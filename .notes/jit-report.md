...
| 3 | `f03df37` | CLI JIT opt-in flag (now the default, `--jitless` opts out) + `--jit-bench` interpreter-vs-JIT benchmark. |
...
| 8 | `e7cb203` | Sweep JIT run (test262 conformance through the JIT; `--jitless` opts out). |
...
| 45 | *(worktree)* | **Fast-argument cap raised to 16** (the `vector leaf call` row): `FAST_CALL_MAX_ARGS` 8 → 16 — plain calls with 9-16 args now take the fast `CallFast`/`TailCallFast` form (one step, the leaf-inline probe) instead of the vector form, whose JIT path ran 11 helper calls per iteration (`ArgsBase` + 9×`ArgsPush` + `Call`). The two `[Value; FAST_CALL_MAX_ARGS]` stack buffers (`do_call_apply`, `run_inline_leaf`) grow with the cap; a 17+-arg call or a spread still takes the vector form, and the vector-form JIT tests that used 10-arg calls switched to 17 to keep their coverage. Measured on the 9-arg shape (200K): interp ~22.5ms → ~14ms, jit ~17.9ms → ~2.4ms (7.4x). New spec-exact unit test; full sweep green. |
| 46 | *(worktree)* | **Prototype-chain member-read cache** (the apply path's member read): `f.apply`'s `apply` lives on `Function.prototype`, so the own-property member cells never serve it and every read paid the full prototype walk. A new agent-level `member_chain_cells` cache stores the resolved chain value for `(receiver, name)`, re-validated by the receiver's generation and each walked link's (id, generation) (an own property on the receiver, a link's mutation, or a proto replacement bumps a generation; links below the found one cannot shadow its own data property, and accessors are never cached) — the `store_chain_clean` verdict pattern generalized to reads. It serves every method read (`f.apply`, `arr.push`, `o.m()`), not just the apply path; the JIT's `GetMemberName` slow helper inherits it with no JIT changes (its inline own-property probe still misses chain reads). Measured on the `apply leaf call` row (200K, A/B): interp ~57ms → ~20ms, jit ~52ms → ~18ms (~2.8x / ~2.9x). New spec-exact invalidation test; full sweep green. |
| 47 | *(worktree)* | **Fast-argument cap raised to 32** (the `vector leaf call` row): `FAST_CALL_MAX_ARGS` 16 → 32 — plain calls with 17-32 args now take the fast `CallFast`/`TailCallFast` form (one step, the leaf-inline probe) instead of the vector form, whose JIT path ran 18 helper calls per iteration (`ArgsBase` + 16×`ArgsPush` + `Call`). The two `[Value; FAST_CALL_MAX_ARGS]` stack buffers (`do_call_apply`, `run_inline_leaf`) grow with the cap; a 33+-arg call or a spread still takes the vector form. Measured on the 17-arg shape (200K, A/B): interp ~28.8ms → ~17.0ms, jit ~21.4ms → ~2.8ms (7.6x). The `--jit-bench` row was bumped to 33 args (interp ~44-48ms, jit ~31-32ms) and the vector-form tests that used 17-arg shapes moved to 33 to keep the vector form covered; `wide_fast_form_calls_stay_spec_exact` covers the 33-arg boundary. Full sweep green in both modes. |

**Measured and reverted: the inline prototype-chain probe (2026-09-01).**
Row 46's cache left the JIT's `GetMemberName` slow helper serving chain
reads; an inline probe (Function `Function.object` hop, cell validation,
live link walk — the machine-code mirror of `member_chain_get`) was
measured **~5-10% slower than the helper** on 20M chain-read loops
(`o.m` 424-440ms → 469-486ms; `f.apply` 620-638ms → 681-716ms, A/B vs
`2b787ff`) and did not move the call-dominated `apply leaf call` row.
LLVM schedules the helper's ~13-16 L1 loads better than the raw
Cranelift probe and the call overhead is tiny, so duplicating the
validation inline cannot win. Reverted; the interp-side cache inside the
...
went **15.9s → 0.26s** under the JIT. Root cause:
...
helper call, so the machine code runs at interpreter speed (the JIT
...
3. **Remaining**: the interpreter's per-step dispatch (~90 ns/step — the
   ...
   store adds a further edge under the JIT), the whole `built-ins/RegExp`
   ...
   fixture passes individually; the sweep runs the JIT by default;
   `test262-sweep --jitless` opts out).
...
1. ~~**`instantiate_accessor`**~~ — **done** (Cut 48): the accessor body
   ...
   accessor-in-a-loop with a per-iteration call: 1100ms → ~82ms under the JIT
...
12. **Make the `buildString` shape fast (array stores + member writes)** —
   ...
   store loop under the JIT (the call_slow wrapper was small; the crux
