...
Phase 7), and reports pass/skip/fail — `44/44` pass, one `#[test]` per fixture (expanded by
the `test262_fixture!` macro) so failures are attributable per file and `cargo test -p test262
<name>` filters a single fixture. The subset grows with each phase's feature coverage.

The workspace runs **387 tests** (`cargo test --workspace`: 112 in `runtime`, 44 in `parser`,
...
`Object.*`, `Function.prototype`). Workspace runs **387 tests** (the test262 fixtures are now
one `#[test]` each) with fmt and clippy (`-D warnings`) clean.
