...
pub fn dispatch_call(
    ...
    if resolved.is(PR_RESOLVED_OPTIONS) {
        return Some(resolved_options_method(agent, this));
    }
    if resolved.is(PR_SELECT) {
        return Some(select_method(agent, this, args));
    }
    if resolved.is(PR_SELECT_RANGE) {
